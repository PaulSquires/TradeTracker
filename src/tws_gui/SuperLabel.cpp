...
//'------------------------------------------------------------------------------ 
LRESULT CALLBACK SuperLabelProc(
    HWND hWnd,
    int uMsg,
    WPARAM wParam,
    LPARAM lParam)

    SUPERLABEL_DATA* pData;

if (uMsg != WM_CREATE) {
    pData = (CAST)(SUPERLABEL_DATA*, GetWindowLongPtr(hWnd, 0));
}
   ...
   Select Case uMsg 
      ...
      case WM_SETCURSOR
         if pData->HotTestEnable = true then
            SetCursor( LoadCursor( NULL, CAST(LPCWSTR, IDC_HAND) ) )
            function = true
            exit function
         end if
      ...
      CASE WM_MOUSEMOVE
         SendMessage( pData->hParent, MSG_SUPERLABEL_MOUSEMOVE, cast(WPARAM, hwnd), 0 )
         ...
         If GetProp( HWnd, "HOT" ) = 0 Then
            ...
            SetProp( HWnd, "HOT", Cast(HANDLE, CTRUE) )
      ...
      case WM_LBUTTONUP   
         PostMessage( pData->hParent, MSG_SUPERLABEL_CLICK, cast(WPARAM, hwnd), 0 )
